...
function getEntryType(dateStr) {
  const entry = (props.entries || []).find(e => e.date.split(' ')[0] === dateStr)
...
<template>
  <div class="rounded-xl shadow-sm p-6 transition-colors duration-200"
    ...
    <div class="grid grid-cols-7 gap-1">
      <div v-for="({ day, date }, index) in calendarDays" :key="index"
        class="aspect-square flex items-center justify-center text-sm rounded-lg" :class="{
